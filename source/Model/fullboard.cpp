...
FullBoard::FullBoard() : space(5, vector<vector<Field> >(5, vector<Field>(5)))
{
	for(int y = 0; y != 5; ++y) {
		for(int x = 0; x != 5; ++x) {
			for(int z = 0; z != 5; ++z) {
				space[y][x][z] = Field(Player::Nobody, Figure::None, Coord(y, x, z));
			}
		}
	}
}

Field& FullBoard::at(const Coord& place) {
...
std::istream& operator>>(std::istream& is, Model::FullBoard& board) {
	for(int y = 4; y >= 0; --y) {
		for(int z = 0; z != 5; ++z) {
			for(int x = 0; x != 5; ++x) {
				board.space[y][x][z] = Field(Player::Nobody, Figure::None, Coord(y, x, z));
